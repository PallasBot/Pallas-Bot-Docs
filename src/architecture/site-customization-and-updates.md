...
> 导航：[`标准部署`](/deploy/deployment) · [`Docker 部署`](/deploy/docker) · [`配置存储`](settings-storage.md) · [`FAQ`](/deploy/faq)
...
4.0 起另支持 **WebUI 社区插件商店**（git 安装到 `local/plugins/`）与 **官方扩展商店**（pip）。见 [社区插件商店](/guide/community-plugin-store)。
