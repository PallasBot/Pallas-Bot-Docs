...
旧 4.0 路线图、阶段验收与分期设计稿不再作为现行入口维护。

处理原则：

1. **仍描述现行事实**：提炼后并入当前开发文档主线。
2. **只剩历史阶段意义**：直接删除或归档。
3. **新入口优先收敛到更少的现行文档**，避免继续维护平行路线图。

## 9. 现行入口

- 本文件：Pallas 的品牌与系统总契约
- [pallas-final-ai-shape.md](pallas-final-ai-shape.md)：Bot ↔ AI 终态边界
- [../developer/index.md](/developer/index)：当前开发文档主线
- [../developer/architecture/overview.md](/developer/architecture/overview)：现行架构入口
- [../common/corpus/README.md](/common/corpus)：社区语料与联邦读取现状
