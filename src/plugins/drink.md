# drink（牛牛喝酒）

群内饮酒与醒酒，持久化醉酒度，影响聊天、轮盘、夺舍、做梦等插件。

## 用户命令

| 口令 | 场景 | 说明 |
| --- | --- | --- |
| 牛牛喝酒 / 牛牛干杯 / 牛牛继续喝 | 群内 | 增加醉酒度，可能睡着 |
| 牛牛醒一醒 / 牛牛别喝了 | 群内 | 立即醒酒；本群在做梦时一并醒梦 |

## 命令权限

无独立命令 ID（口令无额外鉴权）。

## 配置

无独立 `config.py`；状态与冷却见 `BotConfig`。

## 分片（多 worker）

喝酒/醒酒口令（`牛牛喝酒` 等）在代码侧**恒 fanout**（见 `src/platform/ingress/drink_plaintext.py`），无需写入 WebUI「入站：全员同响口令」白名单；各牛独立醉酒态。

## 排障

| 现象 | 处理 |
| --- | --- |
| 喝酒无反应 | 群冷却内可能静默；查日志是否 `ActionFailed`；分片下若见 `Event ... is ignored` 且非饮酒口令，多为 ingress claim |
| 仅一头牛喝酒 | 历史版本需白名单；现版应全牛同响，确认 worker 已更新 |
| 一直不醒 | 发 `牛牛醒一醒`；或等待定时 `fully_sober_up` |

## 实现

[`src/plugins/drink/`](https://github.com/PallasBot/Pallas-Bot/tree/main/src/plugins/drink/)
