...
4. 确认 [`config/pallas.toml`](https://github.com/PallasBot/Pallas-Bot/blob/main/config/pallas.example.toml) 中连接信息可用（`[bootstrap]` / `[bootstrap.mongo]` / `[bootstrap.postgres]`），或从旧 `.env` 运行 `tools/migrate_env_to_pallas.py`。
