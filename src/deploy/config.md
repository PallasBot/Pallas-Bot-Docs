...
::: tip 导航
[配置存储](/architecture/settings-storage) · [标准部署](/deploy/deployment) · [Docker 部署](/deploy/docker) · [FAQ](/deploy/faq)
:::
...
Docker Compose 升级栈中 Bot 容器内 host 为 **`mongodb`**（由 compose 注入），见 [Docker 部署](/deploy/docker)。
:::
...
恢复时保持路径与挂载一致；Docker 见 [配置存储 · Docker 挂载](/architecture/settings-storage)。
:::
...
| 我想… | 文档 |
| --- | --- |
| 分步部署验收 | [标准部署](/deploy/deployment) |
| Docker 挂载与卷 | [Docker 部署](/deploy/docker) |
| 合并顺序与热重载 | [配置存储](/architecture/settings-storage) |
| 站点插件与扩展 | [站点定制](/architecture/site-customization-and-updates) |
