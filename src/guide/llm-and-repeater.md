...
- **复读 / 接话**：看本机学到的群聊语料，决定要不要接  
- **`@牛牛` 闲聊**：明确叫牛说话，走 `llm_chat`
...
这是 [`repeater`](/plugins/repeater) 的能力。
...
这是 [`llm_chat`](/plugins/llm_chat) 的能力。
...
因为 Pallas-Bot 不是把「复读」和「AI」完全割裂开的。
...
也就是说，会先尽量在已有群语料上做选择和轻改写，只有前面的 grounded 路径都没走通时，才让 fallback 生成补位。
...
- 想看 `@牛牛` 怎么用：[`llm_chat`](/plugins/llm_chat)
- 想看平时接话 / 复读：[`repeater`](/plugins/repeater)
- 想看牛格和群味怎么来的：[`persona`](/plugins/persona)
