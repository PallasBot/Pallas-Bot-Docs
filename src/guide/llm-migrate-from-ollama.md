...
4. 在 WebUI **AI 配置 → 对话** 打开 **`LLM_CHAT_ENABLED`**（启用智能对话）；需要遗留 RWKV 时再开 **`CHAT_ENABLE`**（启用遗留酒后 RWKV）。填写 `AI_SERVER_HOST` / `AI_SERVER_PORT`（媒体 / RWKV 用）。
5. 删除或注释遗留 `OLLAMA_*`；**`CHAT_ENABLE` 可保留**（只要还要用 AI 仓 RWKV 酒后路径）。
6. 验收：日志出现 Provider / LLM 正常；群内 @ 与（可选）接话 polish/fallback 正常；若保留 RWKV，醉酒且关 LLM 总闸时仍能走 `/api/chat`。
