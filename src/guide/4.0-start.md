...
默认只加载 **core**。玩法 / 协议端 / AI 媒体走 **官方扩展**；智能接话还要 **Pallas-Bot-AI**。
...
完整键表以 [配置存储](/architecture/settings-storage) 与对应插件文档为准。
...
完整步骤与已知限制见 **[从 3.x 迁到现行版本](4.0-migration.md)**。简要：

1. 备份 `config/pallas.toml`、`data/`、`webui.json`
2. 更新到你要部署的版本（`git pull` 或按 Release 检出 tag）
...
| 项 | 说明 |
| --- | --- |
| PyPI `pallas-core` | `./scripts/build_core.sh` 可本地构建 wheel；正式包以发布为准 |
| WebUI i18n | 中文为主 |
| streaming @聊天 | 非流式分段输出 |

OpenAPI codegen、Setup Wizard、商店安装 SSE、日志续传等已可用，详见 [迁移指南 · 已知限制](4.0-migration.md#已知限制)。
...
| [从 3.x 迁到现行版本](4.0-migration.md) | 升级与已知限制 |
| [CHANGELOG](https://github.com/PallasBot/Pallas-Bot/blob/main/CHANGELOG.md) | 变更摘要 |
| [Pallas-Bot 核心契约](/architecture/internal/pallas-core-contract) | 语料底盘、牛格、群味、多牛社交总纲 |
| [AI 终态架构](/architecture/internal/pallas-final-ai-shape) | Bot↔AI 契约 |
