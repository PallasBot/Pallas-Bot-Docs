...
五分钟跑通仍看 [五分钟跑起来](/guide/quickstart)。  
要装决斗、协议端、MAA，或随时闲聊 / 记忆，继续往下。

默认只加载 **core**。玩法 / 协议端 / AI 媒体走 **官方插件**；智能接话还要 **Pallas-Bot-AI**。

::: tip 先别开分片
第一次：单进程 + 一个 QQ 号就够。先让牛连上 QQ、群里能回话。
:::
...
完整键表以 [配置存储](/developer/architecture/config-storage) 与对应插件文档为准。

---

## 第 3 步：接上 Pallas-Bot-AI

与主仓 **同级目录** 克隆 [Pallas-Bot-AI](https://github.com/PallasBot/Pallas-Bot-AI)，使用与 Bot 匹配的版本。
...
| 全官方插件 | `perf,pg,deploy-all` |
| 分片 | 再加 `deploy-shard`，并配置 `REDIS_URL` |

见 [Docker 部署](/deploy/docker)。单机先跑通再考虑分片。

---

## 从 3.x 升级？

完整步骤见 **[从 3.x 迁到 4.0](4.0-migration.md)**。多数站点**无痛升级**，简要：

1. 备份 `config/pallas.toml`、`data/`、`webui.json`
2. `uv run pallas maintenance run --update-bot --update-webui --dev`（或见 [迁移指南](4.0-migration.md) 分步命令）
3. 使用 AI 时：`Pallas-Bot-AI` 仓 `git pull` + `uv run pallas ai setup`
4. 已装的官方插件与 `local/plugins/` **不必重装**；仅补以前没开过的玩法
5. 仍用旧 `.env` 主配置时，对照 [ollama → llm_chat](llm-migrate-from-ollama.md) 检查 `LLM_*` 键名
...
| 文档 | 内容 |
| --- | --- |
| [五分钟跑起来](/guide/quickstart) | 最少跑通 |
| [运维 · LLM 与 AI](/maintainer/operate/llm-and-ai) | 闲聊 / 记忆排障 |
| [从 3.x 迁到 4.0](4.0-migration.md) | 无痛升级步骤 |
| [CHANGELOG](https://github.com/PallasBot/Pallas-Bot/blob/main/CHANGELOG.md) | 变更摘要 |
| [架构总览](/developer/architecture/overview) | 运行时分层与归属边界 |
| [LLM 与 AI 运维](/maintainer/operate/llm-and-ai) | Bot↔AI 联调与排障 |
| [ollama 迁移](llm-migrate-from-ollama.md) | `CHAT_ENABLE` / `OLLAMA_*` → `LLM_*` |
| [`@牛牛`、复读接话与 LLM 的关系](llm-and-repeater.md) | 两条路径怎么协同 |
| [插件开发入门](/developer/plugin-development/getting-started) | 写扩展插件 |
