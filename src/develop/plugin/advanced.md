...
完整字段与自检清单：[cmd_perm 接入说明](/common/cmd_perm)
...
可在 `PluginMetadata.extra["command_limits"]` 声明默认 CD（与 `command_permissions` 并列）。详见 [command_limits](/common/command_limits)。
...
详见 [WebUI 插件配置](/common/webui)

## 消息审查（message_scrub）

复读、做梦等插件的入站消息可先经统一审查过滤（广告、风控等）。插件侧按 [message_scrub](/common/message_scrub) 登记 hook，避免在各插件重复实现。

## 跨插件与内核层

可复用能力优先沉淀到 `src/` 内核层（见 [common-layers](/developer/architecture/overview)）：
...
见 [站点定制与更新](/maintainer/deploy/upgrade)
...
见 [多进程分片](../../maintainer/deploy/sharded.md)、[中央入站调度](/developer/architecture/shard-runtime)
