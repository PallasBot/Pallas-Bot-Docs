...
**中央入站调度**在 **unified** 与 **worker** 进程内，于 NoneBot 原生 matcher 之上做预筛选、并发预算与出站整形，把单进程可承载的牛数尽量拉高。仍不足时，按 [多进程分片](bot-process-sharding) 拆 hub + worker。
...
渲染与 Playwright 仍走现有 **media_cache** 队列，与 message_load 联动。[Pallas-Bot-AI](https://github.com/PallasBot/Pallas-Bot-AI) 保持外置服务，ingress 层 remote lane 负责限流。
...
分片下的跨 worker 指标见 [多进程分片 · WebUI 与日志](bot-process-sharding#webui-与日志hub) 中的 `shard-observability`。
...
- fanout、主持牛、独占活动等行为仍由 **ingress_gate** 与各插件 metadata 决定，见 [多进程分片 · ingress_fanout](bot-process-sharding#ingress_fanout-元数据插件侧声明)。
