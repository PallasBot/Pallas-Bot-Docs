...
worker 会自动加载除 `src/plugins` 以外的 `plugin_dirs`、`pallas.toml` 的 **`extra_plugin_dirs`**（推荐 `local/plugins`）以及上表中的模块。详见 [站点定制与更新](site-customization-and-updates.md)。实现见 `src/common/bot_runtime/plugin_loader.py`。
