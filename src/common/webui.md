...
可参考 [`src/plugins/pallas_image/config.py`](https://github.com/PallasBot/Pallas-Bot/tree/main/src/plugins/pallas_image/config.py)。
...
可参考 [`src/plugins/pallas_image/config.py`](https://github.com/PallasBot/Pallas-Bot/tree/main/src/plugins/pallas_image/config.py) 中的 `on_pallas_image_config_reload`。
...
若希望插件配置出现在「通用配置」列表（而非仅「插件」页），在 [`env_sections.py`](https://github.com/PallasBot/Pallas-Bot/tree/main/src/console/webui/env_sections.py) 的 `_registered_sections()` 中增加 `_plugin_env_section_from_module(...)`。已 `install_hot_reload_config` 的插件会通过注册表读取当前值；保存段配置时会尝试 `reload_plugin_config`。

### 服务网关 / 连通性（`service_gateways`）

[`service_gateways_section.py`](https://github.com/PallasBot/Pallas-Bot/tree/main/src/console/webui/service_gateways_section.py) 聚合 **牛牛画画** 网关字段、**MAA** 对外端点、**点歌** 主机与开关，对应键写入 `webui.json` 的 `env`（按插件分组到 `sections`）。WebUI 提供网关列表编辑器与 `POST /common-config/service_gateways/connectivity-check`（可按表单草稿探测，无需先保存）。各插件 **插件配置** 页保留完整参数与专用入口（如画画仅测画画网关）。

## 实现参考

| 插件 | 说明 |
|------|------|
| [`sing`](/plugins/sing) | 标准 `install_hot_reload_config` |
| [`pallas_image`](/plugins/pallas_image) | 自定义解析 + `on_reload` |
| [`pallas_webui`](/plugins/pallas_webui) | 调用 `plugin_api` / `env_sections` 的 HTTP 层 |
