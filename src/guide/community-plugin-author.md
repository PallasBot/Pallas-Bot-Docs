...
站点管理员安装说明见 [社区插件商店](community-plugin-store.md)；插件结构见 [Golden Plugin](/developer/plugin-development/golden-plugin)。
...
可选：接入 [cmd_perm](/common/cmd_perm) 的 `command_permissions`，帮助图会自动展示「何人可用」。
...
包内候选路径与 URL 规则见 [插件目录约定 · 包内视觉资源](/architecture/plugin-convention#插件包内视觉资源assets)。
...
| 站点 `local/plugins` | [站点定制](/architecture/site-customization-and-updates) |
