...
> - [ ] 能跑通 `uv sync` / `uv run nb run` 这套主仓默认命令  
...
`uv` 是 [Astral](https://astral.sh/) 出的**新一代 Python 工具链**（Rust 写的）。**Pallas-Bot 的官方部署文档全用 uv**——`uv sync --extra pg`、`uv run nb run` 这些命令你早晚要见。
...
uv sync --extra pg         # 装常规 + 名叫 "pg" 的额外依赖组
...
# 4. 同步依赖(用 PostgreSQL 的话加 --extra pg)
uv sync --extra pg
...
- [ ] 能照着主仓 [部署文档](/deploy/deployment) 跑通 `uv sync --extra pg` + `uv run nb run`
