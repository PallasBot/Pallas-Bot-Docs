...
**Pallas-Bot 不直接登录 QQ。**  
路径是：`QQ ←→ NapCat（协议端）←→ Pallas-Bot ←→ 数据库`。  
...
1. 打开协议端页并登录（口令与 `/pallas/` 相同）  
2. **新建实例** → 选 **NapCat** → 扫码  
3. 实例列表为 **在线**，WS 一般指向 `ws://127.0.0.1:8088/onebot/v11/ws`
...
Bot 与 NapCat 不在同一台时，不要写 `127.0.0.1`（除非同机）。
...
▶ [安装官方扩展](install-extensions.md) · [网页控制台](web-console.md) · [排障](/maintainer/operate/troubleshooting)
