...
- 使用 AI 时，另在 Pallas-Bot-AI 仓 `git pull` 后执行 `uv run pallas ai setup`
- 官方插件一般随已装 pip 保留，**不必因升 4.0 重装**；需对齐 PyPI 新版时在控制台 **插件商店** 点「更新」
...
别默认「主仓升级后官方插件一定兼容旧状态」。在控制台 **插件商店** 查看版本与「更新」即可；仅无 UI 时用 `pallas ext install` 覆盖安装。
...
4. 与官方插件 / 内置插件同名时 **优先加载 local**

社区插件商店与官方插件商店见 [社区插件商店](../../guide/community-plugin-store.md)、[安装插件](../../guide/install-plugins.md)。
...
同时使用 local 整包覆盖与官方插件时，可能出现同一口令被多个模块注册。生产建议：
