...
> 导航：[`README`](https://github.com/PallasBot/Pallas-Bot/blob/main/README.md) · [`Docker 部署`](/deploy/docker) · [`配置要点`](/deploy/config) · [`多进程分片`](/architecture/bot-process-sharding) · [`3.0 迁移`](/about/migration) · [`FAQ`](/deploy/faq)
...
> **勿关闭运行 Bot 的终端**（未配置守护时关闭即停止服务）。Linux 生产环境请使用下文 **systemd** 或 [Docker](/deploy/docker)。
...
详见 [`pallas_protocol` 插件说明](/plugins/pallas_protocol)。
...
- 若必须公网访问控制台，请配合反向代理、HTTPS 与强口令；`ACCESS_TOKEN` 见 [配置要点](/deploy/config)。
...
与 [README 社区区块](https://github.com/PallasBot/Pallas-Bot/blob/main/README.md#qq-群) 保持一致。
