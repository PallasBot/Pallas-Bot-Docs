...
> 与**社区共享接话库**无关：在线统计默认开启；共享语料默认关闭，见 [社区共享接话库](/common/corpus)。
...
| 共享语料 | 关闭 | 在社区共享接话库中单独开启 |
| 牛牛名册公开 | **头像昵称开启**（QQ 默认关） | 社区主站气泡墙展示昵称与在线状态 |
| 语料热词 | 主站默认高频池 | 近期活跃为日/周/月窗口；机群叠加见 `mode=fleet` |
...
| `roster_public_profile` | `true` | 是否在主站公开牛牛头像昵称 |
...
默认上报昵称与头像；QQ 默认不公开。不上报群号或消息正文。开启 QQ 公开后主站气泡墙会展示资料卡链接。
