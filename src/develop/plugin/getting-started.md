...
Pallas-Bot 基于 **NoneBot2** + **OneBot v11**。业务插件位于 `src/plugins/<包名>/`；站点定制可放在 `local/plugins/`（见 [站点定制](/maintainer/deploy/upgrade)）。
...
业务代码通过 `get_my_config()` 读取，勿缓存 import 时的配置快照。详见 [WebUI 插件配置](/common/webui)。
...
- 规范总览：[插件目录约定](/developer/plugin-development/golden-plugin)
