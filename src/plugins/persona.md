...
无单独命令。接话时的语气、长度、活跃度与群内风格会按学习结果自动生效。
...
| 专属自称 | 登录昵称 / 群名片、学到的外号；复合名会拆短别名（如「豆包牛牛」→「豆包」） | 明文点名（如「帕拉斯出」「豆包」）时，其它号应让出 |

说明：

- 「帕拉斯」等产品名**不会**再默认塞给每一只号；只有真正挂上该专属自称的号才会认。
- 「漂亮牛牛」这类**前面粘字**的复合名，不会只因含「牛牛」就当通称点名全员（通称只卡前界）。
- 「牛牛测试机」等句首专名：通称会命中；应用该号的长别名/点名路由做独占，而不是靠后界名单。
- 教称呼仍可用既有说法（如「××就是你」）；学到的专属自称会进自称注入与点名路由。

同群多号听感还会受自动牛格指纹（按账号 archetype / seed 的短接话约束）与表达库「本号优先、短窗少滚同一母题」影响，不只靠数值轴。

自动牛格是账号级人格/表达指纹，不承载临时情绪。怒气是按 Bot、群和用户隔离的短期状态，好感是长期关系状态；攻击导致的怒气或好感变化都不会改写 archetype、seed 或牛格指纹。
...
| 现象 | 处理 |
| --- | --- |
| 叫专属自称仍多号一起回 | 确认目标号登录名/已学专属自称是否正确；通称「牛牛」本身不独占 |
| 叫短名点不中（如「豆包」） | 看登录昵称是否为「××牛牛」类复合名（应能拆短）；或教一次专属自称 |
