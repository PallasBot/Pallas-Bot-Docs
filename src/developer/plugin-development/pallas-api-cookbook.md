...
扩展作者稳定入口。实现在 `pallas.core` / `pallas.product`；**社区扩展只允许 import `pallas.api.*`**（及模板约定的包内模块）。官方扩展可用 `pallas.api.platform`（见 [Platform API](/developer/reference/platform-api)）。
...
- [包布局与 api 白名单](/architecture/internal/pallas-package-layout)
