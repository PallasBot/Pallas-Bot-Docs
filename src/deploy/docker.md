...
> 导航：[`README`](https://github.com/PallasBot/Pallas-Bot/blob/main/README.md) · [`标准部署`](/deploy/deployment) · [`配置要点`](/deploy/config) · [`多进程分片`](/architecture/bot-process-sharding) · [`3.0 迁移`](/about/migration) · [`FAQ`](/deploy/faq)
...
**如何确认成功**：控制台显示在线 Bot；QQ 内测试指令有回复。详见 [`pallas_protocol`](/plugins/pallas_protocol)。
...
数据卷曾用其他 `POSTGRES_DB` 初始化，与当前 `PG_DB` 不一致。对齐库名或清空 `./postgres/data` 后重建，见原文档说明或 [FAQ](/deploy/faq)。
