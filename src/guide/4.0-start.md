...
完整键表见 [persona-llm-roadmap](/architecture/persona-llm-roadmap)、[配置存储](/architecture/settings-storage)。
...
2. 切 `dev` 或等待 4.0.0 tag
3. `uv sync` 并安装所需 `--extra`
4. 将 `CHAT_ENABLE` / `OLLAMA_ENABLE` 迁移为 **`LLM_CHAT_ENABLED`**
5. 确认原内置玩法已装对应扩展包（见 [本体瘦身](../architecture/pallas-4.0-slim.md) 对照表）
...
| [4.0 路线图](../architecture/pallas-4.0-roadmap.md) | 牛格 + 瘦身总览 |
| [本体瘦身](../architecture/pallas-4.0-slim.md) | core / extra / 安装路径 |
| [Bot ↔ AI 仓](/architecture/pallas-ai-service) | API 契约 |
| [接话 LLM 路线图](/architecture/persona-llm-roadmap) | persona / fallback |
| [方舟 KB](/architecture/arknights-knowledge-mcp) | 干员查询与 tools |
| [插件 Cookbook](/develop/plugin/cookbook) | 写扩展插件 |
