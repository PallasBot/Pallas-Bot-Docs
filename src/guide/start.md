...
Pallas-Bot（牛牛）是面向群聊场景的学习型机器人。本节按**检查清单 + 分步**完成最小可用部署；生产环境与 Docker 见专门文档。
...
- [ ] 已复制并编辑 **`config/pallas.toml`**（超管 QQ、数据库）
- [ ] 数据库服务可连接
...
**确认**：目录中存在 `pyproject.toml`、`config/pallas.example.toml`。
