...
牛牛由四块组件拼起来：**协议端**替 QQ 登录收消息，**Pallas-Bot** 处理消息（复读、玩法、插件），**数据库**存语料与状态，**网页控制台**管配置。日常改配置找哪一块，看下表即可。
...
| 网页控制台 | 改配置、看日志、装插件（路径 `/pallas/`） |

## 通信路径

![通信路径：QQ ↔ 协议端 ↔ Pallas-Bot ↔ 数据库，网页控制台与 Bot 同进程](/assets/concepts-topology.svg)

- 协议端与 Pallas-Bot 通过 **OneBot WebSocket** 通信。
- 网页控制台与协议端管理页由 **同一 Bot 进程** 提供，无需另起服务。
...
::: details 进阶：消息怎么在牛牛中流动
一条群消息进入统一运行时后，并不是立刻交给全部插件逐个尝试。牛牛会先完成轻量判断：已经接入公共运行时 API 的精确命令走 `direct` 短路径，复杂交互和其他插件继续走 NoneBot `matcher`。两条路径完成处理后共用既有出站与任务设施：

![Pallas-Bot 统一消息入口：精确命令走 direct，复杂交互和兼容插件走 matcher，两条路径最终共用出站与任务设施](/assets/message-runtime-architecture.svg)

### 1. 入站门控与路径选择

牛牛会跳过自己或已知其他牛发出的群消息；消息若 `@` 了某些牛牛账号，则只交给被 `@` 的账号。普通消息会经本机、联邦或分片领取，未领取到的实例跳过；全员响应玩法会绕过领取竞争，让符合目标条件的账号都收到事件。
...
- 能说明：群消息先经协议端，再到牛牛；控制台改的是同进程配置。
- 知道改端口 / 超管找 `pallas.toml`，改插件与 AI 等日常项找控制台。
- 知道单机优先使用统一运行时，只有部署边界需要扩展时再用分片。

## 接下来做什么

- 第一次启动见 [快速开始](/guide/quickstart)
- 连接 QQ 见 [连接 QQ](connect-qq.md)
- 配置落盘规则见 [配置从哪改](config.md)
