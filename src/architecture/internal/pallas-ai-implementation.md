...
| 7.6 | **memory infra** | 运行时 session / task state 在 AI 仓；产品记忆策略仍在 Bot（final §2.4） | 已落地（Bot：hybrid 检索 + embedding 缓存、启发式 auto_episode、`memory.search`/`memory.save` tools、`data/pallas_knowledge` ingest；见 [llm-and-ai.md](/maintainer/operate/llm-and-ai)） |
