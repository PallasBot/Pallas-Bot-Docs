...
接着回到主流程，照 [快速部署](/guide/quickstart) 把 bot 的依赖装上、配置写好、跑起来。Windows 上守护进程别用 `systemd`（没这玩意儿），用[任务计划程序](https://learn.microsoft.com/zh-cn/windows/win32/taskschd/about-the-task-scheduler) 或 [NSSM](https://nssm.cc/) 当替代——这是另一坨坑了，建议**真到那一步再迁 Linux**。
