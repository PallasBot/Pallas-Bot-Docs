...
worker 会自动加载除 `src/plugins` 以外的 `plugin_dirs`、[`pallas.toml`](https://github.com/PallasBot/Pallas-Bot/blob/main/config/pallas.example.toml) 的 **`extra_plugin_dirs`**（推荐 `local/plugins`）以及上表中的模块。详见 [站点定制与更新](site-customization-and-updates.md)。实现见 `src/common/bot_runtime/plugin_loader.py`。

部署入口见 [标准部署 · 多进程分片](/deploy/deployment#多进程分片可选) 与 [Docker 部署 · 多进程分片](/deploy/docker#多进程分片可选)。
...
- 宿主机 `bot_watchdog.py` **仅监护 hub** 时须 **`--no-spawn`**，且 worker 由同一套编排保证拉起（见 [进程守护脚本](/deploy/deployment#进程守护脚本)）。

## 配置与进程角色

运行配置以 **[`config/pallas.toml`](https://github.com/PallasBot/Pallas-Bot/blob/main/config/pallas.example.toml)** + **`data/pallas_config/webui.json`** 为主（见 [配置存储](settings-storage.md)）；根目录 **`.env` 仅遗留只读合并**，WebUI 不再写入。分片编排仍可用 `run_sharded_bot.sh` 的 `env KEY=val` 注入 **`PALLAS_BOT_ROLE`、`PALLAS_SHARD_ID`、`PORT`** 等，**优先于磁盘同名项**。

### Hub / Worker 常用项

写在 [`pallas.toml`](https://github.com/PallasBot/Pallas-Bot/blob/main/config/pallas.example.toml) 的 `[env]`（或 WebUI「通用配置」），或由启动脚本注入：
...
与 **Pallas-Bot-AI** 共用 Redis 时，可减少 `data/*/message_claims` 的磁盘竞争（ingress / 插件抢占）。在 **[`config/pallas.toml`](https://github.com/PallasBot/Pallas-Bot/blob/main/config/pallas.example.toml) 的 `[env]`** 中配置（勿再依赖根目录 `.env`）：
...
`./scripts/run_sharded_bot.sh start` 会读取 **[`pallas.toml`](https://github.com/PallasBot/Pallas-Bot/blob/main/config/pallas.example.toml) / `webui.json`** 并调用 `detect_shard_redis.py` 自动探测；不可达时回退文件 claim。
...
| 项 | 说明 |
|----|------|
| **[`config/pallas.toml`](https://github.com/PallasBot/Pallas-Bot/blob/main/config/pallas.example.toml)** | 各进程同一份（`[bootstrap]` 数据库与监听；插件项多在 `webui.json`） |
...
示例编排见仓库 [`docker-compose.shard.example.yml`](../../docker-compose.shard.example.yml)（hub + 多个 worker，**共用** [`./pallas-bot/config/pallas.toml`](https://github.com/PallasBot/Pallas-Bot/blob/main/config/pallas.example.toml) 与 `./pallas-bot/data`）。
...
完整步骤见 [Docker 部署 · 多进程分片](/deploy/docker#多进程分片可选)。
...
- `GET /pallas/api/shard-observability`：合并各 worker `stats/worker-*.json` 的 **ingress 命中率**、**coord 积压**（实时扫描 `coord/`）、**PG 连接池粗算**；**WebUI 首页**在分片模式下展示「分片可观测」面板。
