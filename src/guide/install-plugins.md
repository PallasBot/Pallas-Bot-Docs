...
::: details 没有「一键安装」？
常见原因：Docker 精简镜像、PATH 里没有 `uv`。改用方法 B，或构建时带上 `PALLAS_UV_EXTRAS`（见 [Docker](/deploy/docker)）。  
...
装完 **必须重启**。包对照：[安装官方插件](install-extensions.md#扩展包对照表)。
...
与官方插件 **同名时 local 优先**。细节：[站点定制](/maintainer/deploy/upgrade)。
